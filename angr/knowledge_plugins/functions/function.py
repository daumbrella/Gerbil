...
class Function(object):
    ...
    def byteStr32(self):
        """
        An sort iterator of all local blocks in the current function.
        :return: angr.lifter.Block instances.
        """
        func_byteStr32=''
        byte_list=self._project.loader.memory.read_bytes(self.addr-1,32)
        for byte in byte_list:
            byte_str=binascii.b2a_hex(byte)
            func_byteStr32+=byte_str
        return func_byteStr32
    ...
    def identify_lib_func(self):
        ...
        match_result=nampa.match_function(self._function_manager.sig,func_buf)
        if match_result[0]:
            function_names=''
    ...
    def string_references(self, minimum_length=2, vex_only=False):
        ...
        for addr in self.local_runtime_values if not vex_only else self.code_constants:
            #print "DEBUG in function.py : the string ref addr "+hex(addr)
            if not isinstance(addr, claripy.fp.FPV) and addr in memory:
                ...
                try:
                    ...
                    if addr not in known_executable_addresses and possible_pointer not in known_executable_addresses:
                        #print "DEBUG in function find a string addr :  "+hex(addr)
                        # build string
                        stn = ""
                        offset = 0
                        ...
                        if current_char == "\x00" and len(stn) >= minimum_length:
                            ...
                            addr_second_str=''
                            addr_second_arrays=self._project.loader.memory.read_bytes(addr,4)
                            for addr_second_byte in addr_second_arrays:
                                addr_second_str=binascii.b2a_hex(addr_second_byte)+addr_second_str
                            print "DEBUG in function a second string addr : "+addr_second_str
                            addr_second=int(addr_second_str,16)
                            stn = ""
                            offset = 0
                            current_char = memory[addr_second + offset]
                            while current_char in string.printable:
                                stn += current_char
                                offset += 1
                                current_char = memory[addr_second + offset]
                            if current_char == "\x00" and len(stn) >= minimum_length:
                                ...
                                if addr_second in memory and addr_second not in function_pointer:
                                    function_pointer.append(addr_second)
                except KeyError:
                    pass
        '''print "DEBUG in function.py the function pointer are: "
    ...
    def local_runtime_values(self):
        ...
        '''if not self._project.loader.main_object.contains_addr(self.addr-1):
            print "DEBUG in function: the project does not contain the function addr "
            return constants'''
        ...
        while len(q) > 0:
            ...
            if node is None:
                print " DEBUG in function: the node does not exist. maybe it's not a block node."
                continue
            missing = set(x.addr for x in list(self.graph.successors(node))) - analyzed
            for succ_addr in missing:
                print "Forcing jump to missing successor: "+succ_addr
                if succ_addr not in analyzed:
                    ...
                    if len(all_successors) > 0:
                        ...
                        print "Could not reach successor: "+succ_addr
    ...
    def _remove_fakeret(self, from_node, to_node):
        if (from_node,to_node) in self.transition_graph.edges():
            self.transition_graph.remove_edge(from_node, to_node)
            self._local_transition_graph = None
        else:
            print "DEBUG in function remove the fakeret : the edage is not in the graph"
