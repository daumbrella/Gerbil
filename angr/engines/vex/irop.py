...
class SimIROp(object):
    ...
    def _op_vector_float_mapped(self, args):
        ...
        for ca in chopped_args:
            if str(type(self._op_float_mapped(rm_part + ca))) == "<type 'NotImplementedType'>":
                continue;
            else:
                chopped_list.add(ca)
        return claripy.Concat(*(self._op_float_mapped(rm_part + ca).raw_to_bv() for ca in chopped_list))
