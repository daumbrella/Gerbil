...
class BackwardSlice(Analysis):
    ...
    # Private methods
    #
    def _find_func_in_cfg(self,block):
        fm_tmp=self._cfg.kb.functions
        find_func=fm_tmp.floor_func(block.addr)
        is_identify=find_func.ident_flag
        if is_identify:
            #print("DEBUG  begin to match the function "+find_func.name)
            find_func.identify_lib_func 
        return find_func 
    ...
    def _construct_control_flow_slice(self, simruns):
        ...
        self.funcs_in_slice=networkx.DiGraph()
        ...
        while stack:
            ...
            if block.addr not in self.chosen_statements:
                ...
                for pred in predecessors:
                    ...
                    self.runs_in_slice.add_edge(hex(pred.addr), hex(block.addr))                   
                    block_func=self._find_func_in_cfg(block)
                    pred_func=self._find_func_in_cfg(pred)
                    if block_func.name!=pred_func.name:
                        self.funcs_in_slice.add_edge(pred_func,block_func)
