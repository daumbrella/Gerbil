...
class EndPath(object):
    def __init__(self, start_state=None, state=None, path_count=None):
        self._start_state=start_state
        self._end_addr=hex(state.addr)
        self._path_count=path_count
        self.one_solve_int=None
        self.one_solve_hex=None
        self.one_solve_str=None
        self.all_constraints=self.get_all_constraints(state)
        self.target_constraints=self.get_target_constraints()
        recv_data_list=self.get_sym_variables(state,"recv_data")
        if len(recv_data_list)>0:
            recv_data=recv_data_list[0]
            self.one_solve_int=state.solver.eval(recv_data,cast_to=int)
            self.one_solve_hex=hex(self.one_solve_int)
            self.one_solve_str=state.solver.eval(recv_data,cast_to=str)

    def __str__(self):
        return "path %s end addr %s:\nall_constraints:\n%s\ntarget_constraints:\n%s\none solve is :\t%s"% (str(self._path_count),self._end_addr,str(self.all_constraints),str(self.target_constraints),str(self.one_solve_hex))
    
    def get_target_constraints(self):
        constraints=list()
        for con in self.all_constraints:
            if "recv_data" in str(con):
                constraints.append(con)
        return constraints

    def get_all_constraints(self,state):
        return state.history.constraints_since(self._start_state)

    def get_sym_variables(self,state,key):
...
class SimulationManager(ana.Storable):
    ...
    def __init__(self, project, active_states=None, stashes=None, hierarchy=None, veritesting=None,
                 veritesting_options=None, immutable=None, resilience=None, save_unconstrained=None,skip_func=False,filter_constraints=False,
                 save_unsat=None, threads=None, errored=None,function_manager=None,category=None, completion_mode=any):
        ...
        # techniques
        self.end_path_list=list()
        if isinstance(active_states, list):
            self._initial_state=active_states[0]
        elif isinstance(active_states, SimState):
            self._initial_state=active_states
        self.start_state=active_states
        self._hooks_step = []
        ...
        try:
            ...
            for hook in self._hooks_step_state:
                ...
                if successor_func is not None:
                    ...
                    if self._filter_constraints:
                        self._state_count+=1
                        #print "state "+str(self._state_count)
                        #print hex(a.addr)
                        ss = self._project.factory.successors(a, **kwargs)
                        #print "successors"
                        #print "flat successors is"
                        #print ss.flat_successors
                        if len(ss.flat_successors)==0:
                            self._path_count+=1
                            end_path=EndPath(state=a,path_count=self._path_count,start_state=self._initial_state)
                            #print "DEBUG in manager the end path is ", str(end_path)
                            self.end_path_list.append(end_path)
                        if self._function_manager is not None:
                            for fs in ss.flat_successors:
                                fs_addr=fs.addr
                                if self._function_manager.contains_addr(fs_addr):
                                    #print "the flat successor is a function"
                                    find_func=self._function_manager.function(fs_addr)
                                    if self._skip_func:
                                        #print "DEBUG in manager skip the func, ",hex(fs_addr)
                                        hook_func=angr.SIM_PROCEDURES[self._category]["add_constraints"]
                                        self._project.hook(fs_addr,hook_func(func_name=find_func.name))
                                    if self._project.is_hooked(fs_addr):
                                        #print "the flat successor is already hooked"
                                        continue
                                    if find_func.ident_flag:
                                        #print "identify the function"
                                        match_result=find_func.identify_lib_func
                                        if match_result[0]:
                                            #print "the funtion names ",find_func.name
                                            for func_name in match_result[1]:
                                                #print "find the lib ",func_name
                                                if angr.SIM_PROCEDURES.has_key(self._category):
                                                    if angr.SIM_PROCEDURES[self._category].has_key(func_name):
                                                        hook_func=angr.SIM_PROCEDURES[self._category][func_name]
                                                        #print "hook the function"
                                                        self._project.hook(fs_addr,hook_func)
                                            if not self._project.is_hooked(fs_addr):
                                                #print "hook the function with add_constraints",fs_addr
                                                hook_func=angr.SIM_PROCEDURES[self._category]["add_constraints"]
                                            self._project.hook(fs_addr,hook_func(func_name=find_func.name))
                        else:
                            ss = self._project.factory.successors(a, **kwargs)
    ...
    def explore(self, stash=None, n=None, find=None, avoid=None, find_stash='found', avoid_stash='avoid', cfg=None,function_manager=None, category=None,num_find=1, step_func=None):
        """
